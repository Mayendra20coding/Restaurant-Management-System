...
class RestaurantOrderManagement:
    def __init__(self, root):
        ...
        ttk.Label(frame, text='Currency:',
                    font=("Helvetica", 12)).grid(row=len(self.menu_items)+1,
                                                    column=0,
                                                    padx=10,
                                                    pady=5)
    ...
    def setup_background(self, root):
        ...
        canvas.pack()
       # original_image = tk.PhotoImage(file="restaurant_bg.png")
    ...
    def update_menu_prices(self, *args):
        ...
        rate = self.exchage_rate if currency == 'INR' else 1
        for item, label in self.menu_items.items():
    ...
    def place_order(self):
        ...
        for item, entry in self.menu_quantities.items():
            ...
            if quantity.isdigit():
                ...
                total_cost += cost
                if quantity > 0:
                   order_summary+=f"{item} : {quantity} * {symbol}{price} = {symbol}{cost}\n"
        if total_cost > 0:
